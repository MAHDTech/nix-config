...
stdenv.mkDerivation (
  ...
  in
  {
    ...
    ++ lib.optionals stdenv.hostPlatform.isLinux [
      copyDesktopItems
      # build-browser-secret.mjs compiles native/browser-secret/main.c with
      # cc + pkg-config libsecret-1 as part of build:desktop (since 0.0.39).
      pkg-config
    ]
    ...
    + lib.optionalString stdenv.hostPlatform.isLinux ''
      # Keyring helper for Chromium cookie import; the unpackaged desktop app
      # looks for it at <dist-electron>/../prod-resources/browser-secret/.
      install -D --mode=755 native/browser-secret/build/*/t3-browser-secret \
