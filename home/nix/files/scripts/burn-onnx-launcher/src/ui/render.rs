...
pub fn ui(f: &mut ratatui::Frame, app: &mut App) {
    ...
    let colls_len = app.current_collections().len();
    if colls_len > 0 && app.selected_collection >= colls_len {
        app.selected_collection = colls_len - 1;
        app.state_coll.select(Some(app.selected_collection));
    }
    ...
    // 3. Drill-down menu (4-level: Category → Vendor → Collection → Model)
    ...
    match app.focused_pane {
        ...
        FocusedPane::Collections => {
            let collections = app.current_collections();
            let items: Vec<ListItem> = collections
                .iter()
                .map(|c| {
                    let text = Line::from(Span::raw(c.as_str())).alignment(Alignment::Center);
                    ListItem::new(vec![space.clone(), text, space.clone()])
                })
                .collect();
            let vendors = app.current_vendors();
            ...
            let list = List::new(items)
                .block(Block::default()
                    .title(Line::from(format!(" Choose a Collection from '{}' ", parent_vendor)).alignment(Alignment::Center))
                    .borders(Borders::ALL)
                    .border_style(Style::default().fg(Color::Green)))
                .highlight_style(Style::default().bg(Color::DarkGray).add_modifier(Modifier::BOLD))
                .highlight_symbol(">> ");
            f.render_stateful_widget(list, list_chunk, &mut app.state_coll);
        }
        FocusedPane::Models => {
            let collections = app.current_collections();
            let collection = if !collections.is_empty() {
                collections[app.selected_collection.min(collections.len().saturating_sub(1))].clone()
            } else {
                "None".to_string()
            };
            ...
            let items: Vec<ListItem> = models
                ...
                .map(|m| {
                    ...
                    let desc = format!("  ╰─ {} ({})", m.repo_id, m.engine);
                    let text = vec![
                        space.clone(),
                        Line::from(name_span).alignment(Alignment::Center),
                        Line::from(Span::styled(desc, Style::default().fg(Color::DarkGray))).alignment(Alignment::Center),
                        space.clone(),
                    ];
                    ListItem::new(text)
                })
            ...
            let list = List::new(items)
                .block(Block::default()
                    .title(Line::from(format!(" Models for {} ", collection)).alignment(Alignment::Center))
                    .borders(Borders::ALL)
                    .border_style(Style::default().fg(Color::Green)))
