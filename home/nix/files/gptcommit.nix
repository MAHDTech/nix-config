{ config, ... }:
{
  home.file = {
    "gptcommit-config" = {
      target = "${config.home.homeDirectory}/.config/gptcommit/config.toml";
      executable = false;
      ...
      text = ''
        ...
        #api_key = "" # Set via GPTCOMMIT__OPENAI__API_KEY
        #model = "grok-4-1-fast-reasoning" # No tokenizer error
        model = "grok-4-0709"
        retries = 3
        proxy = ""
        ...
        You are an expert programmer summarising a code change.
        ...
        Please summarise the commit into a single specific and cohesive theme.
        ...
        """
        file_diff = """
        You are an expert programmer summarising a git diff.
