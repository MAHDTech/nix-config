...
const AppItem = (address: string) => {
  ...
  const btn = PanelButton({
    ...
    child: Widget.Icon({
      size: iconSize.bind(),
      icon: monochrome.bind().as((m) => icon((app?.icon_name || client.class) + (m ? "-symbolic" : ""), icons.fallback.executable + (m ? "-symbolic" : ""))),
    }),
  });
    ...
    setup: (w) =>
      w
        .hook(
          hyprland,
          (w, address?: string) => {
            if (typeof address === "string") w.children = w.children.filter((ch) => ch.attribute.address !== address);
          },
          "client-removed",
        )
        .hook(
