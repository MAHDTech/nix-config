...
const { autotheme: at, font, theme, bar: b, launcher: l, overview: ov, powermenu: pm, quicksettings: qs, osd, hyprland: h } = options;
...
export default [
  Page(
    "Theme",
    icons.ui.themes,
    Group("", Wallpaper() as ReturnType<typeof Row>, Row({ opt: at, title: "Auto Generate Color Scheme" }), Row({ opt: scheme, title: "Color Scheme", type: "enum", enums: ["dark", "light"] })),
    Group(
      "Dark Colors",
  ...
  Page(
    ...
    Group("Workspaces", Row({ opt: b.workspaces.workspaces, title: "Number of Workspaces", note: "0 to make it dynamic" })),
    Group(
      "Taskbar",
  ...
  Page(
    "General",
    icons.ui.settings,
    Group("Hyprland", Row({ opt: h.gapsWhenOnly, title: "Gaps When Only" }), Row({ opt: h.inactiveBorder, type: "color", title: "Inactive Border Color" })),
    Group("Launcher", Row({ opt: l.width, title: "Width" }), Row({ opt: l.apps.iconSize, title: "Icon Size" }), Row({ opt: l.apps.max, title: "Max Items" })),
    Group(
      "Overview",
    ...
    Group("Powermenu", Row({ opt: pm.layout, title: "Layout", type: "enum", enums: ["box", "line"] }), Row({ opt: pm.labels, title: "Show Labels" })),
    Group(
      "Quicksettings",
