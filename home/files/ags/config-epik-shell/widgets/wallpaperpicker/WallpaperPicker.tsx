...
function getWallpaperList(path: string) {
  ...
  const fileEnum = dir.enumerate_children("standard::name", Gio.FileQueryInfoFlags.NONE, null)
...
function cacheImage(inputPath: string, cachePath: string, newWidth: number, customName?: string, fastest?: boolean) {
  ...
  try {
    ...
    const scaledPixbuf = pixbuf.scale_simple(newWidth, scaledHeight, fastest ? GdkPixbuf.InterpType.NEAREST : GdkPixbuf.InterpType.BILINEAR)
    ...
    const black_pixbuf = GdkPixbuf.Pixbuf.new(GdkPixbuf.Colorspace.RGB, true, 8, newWidth, (newWidth * 9) / 16)
...
function wallpaperPicker() {
  ...
  return (
    <PopupWindow
      ...
      visible
      setup={(self) => {
        sendBatch([`layerrule animation slide ${options.bar.position.get()}, ${self.namespace}`, `layerrule blur, ${self.namespace}`, `layerrule ignorealpha 0.3, ${self.namespace}`])
    ...
    >
      ...
      >
        ...
        <Gtk.ScrolledWindow>
            ...
            setup={(self) => {
              function populateBox(box: Astal.Box, path: string) {
                timeout(100, () => {
                  ...
                  const wallpapersToCache = wallpaperList.filter((image) => !GLib.file_test(`${cachePath}/${image}`, GLib.FileTest.EXISTS))
                  ...
                  box.set_children(
                    wallpaperList.map((w) => (
                      <button
                        tooltipText={w}
                        onClicked={() => {
                          sh(["swww", "img", "--transition-type", "random", `${path}/${w}`]).then(() => {
                            const current = cacheImage(`${path}/${w}`, cachePath, 450, `${w.split(".").shift()}_current`)
                            GLib.remove(wallpaper.current.get())
                            wallpaper.current.set(current)
                          })
                        }}
                      >
                        <Gtk.Picture cssClasses={["image"]} overflow={Gtk.Overflow.HIDDEN} contentFit={Gtk.ContentFit.COVER} widthRequest={200} file={Gio.file_new_for_path(`${cachePath}/${w}`)} />
                      </button>
                    ))
                  )
                })
          ...
          >
            <label label={"Caching wallpapers..."} hexpand halign={Gtk.Align.CENTER} />,
          </box>
        </Gtk.ScrolledWindow>
      </box>
    </PopupWindow>
...
export function toggleWallpaperPicker() {
  const windowExist = App.get_windows().some(({ name }) => name == "wallpaperpicker")
  if (!windowExist) {
    wallpaperPicker()
  } else {
