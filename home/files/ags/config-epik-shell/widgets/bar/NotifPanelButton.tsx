...
function NotifIcon() {
  const getVisible = () => (notifd.dont_disturb ? true : notifd.notifications.length <= 0)
  ...
  return (
    <image onDestroy={() => visibility.drop()} visible={visibility()} cssClasses={["icon"]} iconName={bind(notifd, "dont_disturb").as((dnd) => `notifications-${dnd ? "disabled-" : ""}symbolic`)} />
  )
}
              ...
              if (n.length > 0) {
                return [
                  ...n.slice(0, 3).map((e) => {
                    ...
                    const fallback = e.app_icon.trim() === "" ? getFallback(e.app_name) : e.app_icon
                    const icon = substitute[e.app_name] ?? fallback
                    return <image iconName={icon} />
                  }),
                  <label visible={n.length > 3} cssClasses={["circle"]} label={""} />,
                ]
              }
              return <NotifIcon />
            })}
