  ...
  return (
    <Gtk.ScrolledWindow vexpand>
      <box vertical hexpand={false} spacing={8}>
        {bind(notifd, "notifications").as((notifs) => notifs.map((e) => <Notification n={e} showActions={false} />))}
        <box halign={Gtk.Align.CENTER} valign={Gtk.Align.CENTER} cssClasses={["not-found"]} vertical vexpand visible={bind(notifd, "notifications").as((n) => n.length === 0)}>
          <image iconName="notification-disabled-symbolic" iconSize={Gtk.IconSize.LARGE} />
...
function NotificationWindow(_gdkmonitor: Gdk.Monitor) {
  return (
    <PopupWindow name={WINDOW_NAME} animation="slide top" layout={layout.get()} onDestroy={() => layout.drop()}>
      <box cssClasses={["window-content", "notifications-container"]} vertical vexpand={false}>
