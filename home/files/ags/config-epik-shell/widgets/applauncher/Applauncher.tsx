    ...
    >
      <box>
        ...
        <box valign={Gtk.Align.CENTER} vertical>
          <label cssClasses={["name"]} ellipsize={Pango.EllipsizeMode.END} xalign={0} label={app.name} />
  ...
  return (
    <overlay cssClasses={["entry-overlay"]} heightRequest={100}>
      <Gtk.ScrolledWindow heightRequest={100}>
        <Picture file={wallpaper.current((w) => Gio.file_new_for_path(w))} contentFit={Gtk.ContentFit.COVER} overflow={Gtk.Overflow.HIDDEN} />
      </Gtk.ScrolledWindow>
      <entry
        type="overlay"
        vexpand
  ...
  return (
    <Gtk.ScrolledWindow vexpand>
      <box spacing={6} vertical>
        ...
        <box halign={Gtk.Align.CENTER} valign={Gtk.Align.CENTER} cssClasses={["not-found"]} vertical vexpand visible={list.as((l) => l.length === 0)}>
          <image iconName="system-search-symbolic" iconSize={Gtk.IconSize.LARGE} />
...
export default function Applauncher(_gdkmonitor: Gdk.Monitor) {
  return (
    <PopupWindow name={WINDOW_NAME} animation="popin 80%">
      <box cssClasses={["window-content", "applauncher-container"]} vertical vexpand={false}>
        <SearchEntry />
        <AppsScrolledWindow />
      </box>
    </PopupWindow>
