  ...
  return (
    <box className="MediaPlayer">
      ...
      <box vertical>
        ...
        <slider visible={bind(player, "length").as((l) => l > 0)} onDragged={({ value }) => (player.position = value * player.length)} value={position} />
        <centerbox className="actions">
          <label hexpand className="position" halign={START} visible={bind(player, "length").as((l) => l > 0)} label={bind(player, "position").as(lengthStr)} />
          <box>
...
export default function MprisPlayers() {
  ...
  return <box vertical>{bind(mpris, "players").as((arr) => arr.map((player) => <MediaPlayer player={player} />))}</box>;
}
