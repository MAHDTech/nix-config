...
const updateVisibility = () => {
  return hyprland.get_workspace(hyprland.get_focused_workspace().id)?.get_clients().length <= 0
}
...
function DockHover(_gdkmonitor: Gdk.Monitor) {
  ...
  return (
    ...
    >
      <box cssClasses={["dock-padding"]} widthRequest={widthVar()} heightRequest={heightVar()}>
      ...
      setup={(self) => {
        hook(self, App, "window-toggled", (_, win) => {
          ...
          if (win.name == "dock") {
            ...
            heightVar.set(getHoverHeight() > size!.height ? size!.height : getHoverHeight())
