...
export function QSMenuButton({ child, iconName, label, setup }: QSMenuButtonProps) {
  return (
...
type QSButtonProps<T extends GObject.Object> = ButtonProps & {
  ...
  connection?: [T | Variable<any> | Opt<any>, keyof T | null, ((arg0: any) => boolean)?]
}

export default function QSButton<T extends GObject.Object>({ iconName, label, setup, onClicked, connection }: QSButtonProps<T>) {
  function getCssClasses(): string[] | Binding<string[]> {
    ...
    return object instanceof Variable ? bind(object).as(computeClasses) : property != null ? bind(object, property).as(computeClasses) : ["qs-button"]
  }

  return (
    <button setup={setup} cssClasses={getCssClasses()} onClicked={onClicked} tooltipText={label}>
