...
export const now = () => GLib.DateTime.new_now_local().format("%Y-%m-%d_%H-%M-%S")
...
export function notifySend({
  ...
  const actionsArray = Object.entries(actions || {}).map(([label, callback], i) => ({
    id: `${i}`,
    label,
    callback,
  }))
...
export function separatorBetween(elements: Gtk.Widget[], orientation: Gtk.Orientation) {
