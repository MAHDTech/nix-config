...
async function initGtk(mode: ThemeMode, reset = false) {
  ...
  const defineColor = (key: string, value: string, alpha = 1.0) => `@define-color ${key} alpha(${value}, ${alpha});`
  ...
  await writeFileAsync(targetDir, gtkVar.join("\n"))
    .then(() => {
      ...
      gsettings.set_string("gtk-theme", `adw-gtk3${mode === "light" ? "" : "-dark"}`)
    })
    .catch(console.error)
}

function shorthand(value: number | number[], length: number): ShorthandProperty {
  if (typeof value === "number") {
    return length <= 2 ? { top: value, right: value } : { top: value, right: value, bottom: value, left: value }
  }
...
function applyOffsets(short: ShorthandProperty, element: typeof theme.bar | typeof theme.window) {
  ...
  short.left! += borderWidth + (shadowHOffset <= 0 ? Math.abs(shadowHOffset) : 0)
...
function defineVar(opt: Opt, type = "string", slice = 2, arrayLength = 4) {
  ...
  const typeChecks = {
    ...
    number_or_array: (val: any) => typeof val === "number" || Array.isArray(val),
  }

  if (!typeChecks[type](value)) {
    throw new Error(`Invalid value, ${opt.id} needs ${type.split("_").join(" ")}`)
  }
