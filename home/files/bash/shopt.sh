...
function shell_options() {
	...
	writeLog "DEBUG" "Setting the Shell Options for Bash using shopt"

	# Confirm bash is actually running
	if ! type shopt >/dev/null 2>&1; then
		writeLog "ERROR" "This function: ${FUNCNAME[0]} needs to be called from within Bash"
		return 1
	fi
	...
	# cdspell
	# If set, minor errors in the spelling of a directory component in a cd command will be
	# corrected. The errors checked for are transposed characters, a missing character, and
	# a character too many. If a correction is found, the corrected path is printed, and
	...
	# interactive shell. If any jobs are running, this causes the exit to be deferred until
	# a second exit is attempted without an intervening command (see Job Control). The shell
	...
	# history entry. This allows easy re-editing of multi-line commands. This option is
	# enabled by default, but only has an effect if command history is enabled
	# (see Bash History Facilities).
	shopt -q -s cmdhist

	# compat31
	# If set, Bash changes its behavior to that of version 3.1 with respect to quoted
	# arguments to the conditional command’s ‘=~’ operator and with respect to
	# locale-specific string comparison when using the [[ conditional command’s ‘<’ and
	# ‘>’ operators. Bash versions prior to bash-4.1 use ASCII collation and strcmp(3);
	...
	# locale-specific string comparison when using the [[ conditional command’s ‘<’ and
	# ‘>’ operators (see previous item) and the effect of interrupting a command list.
	# Bash versions 3.2 and earlier continue with the next command in the list after
	# one terminates due to an interrupt.
	shopt -q -u compat32

	# compat40
	# If set, Bash changes its behavior to that of version 4.0 with respect to
	# locale-specific string comparison when using the [[ conditional command’s ‘<’ and
	# ‘>’ operators (see description of compat31) and the effect of interrupting a command
	# list. Bash versions 4.0 and later interrupt the list as if the shell received the
	...
	# compound array assignment as an argument to declare, makes word expansion errors
	# non-fatal errors that cause the current command to fail (the default behavior is to
	# make them fatal errors that cause the shell to exit), and does not reset the loop
	# state when a shell function is executed (this allows break or continue in a shell
	...
	if [ "${BASH_VERSINFO:-0}" -ge 5 ]; then
		...
		# compat44
		# If set, Bash saves the positional parameters to BASH_ARGV and BASH_ARGC before they
		# are used, regardless of whether or not extended debugging mode is enabled.
		shopt -q -u compat44
	...
	# expansion. The filenames ‘.’ and ‘..’ must always be matched explicitly, even if
	# dotglob is set.
	shopt -q -s dotglob

	# execfail
	# If this is set, a non-interactive shell will not exit if it cannot execute the file
	# specified as an argument to the exec builtin command. An interactive shell does not
	# exit if exec fails.
	shopt -q -u execfail

	# expand_aliases
	# If set, aliases are expanded as described below under Aliases, Aliases. This option
	# is enabled by default for interactive shells.
	shopt -q -s expand_aliases

	# extdebug
	# If set at shell invocation, or in a shell startup file, arrange to execute the
	# debugger profile before the shell starts, identical to the --debugger option. If
	# set after invocation, behavior intended for use by debuggers is enabled:
	shopt -q -u extdebug

	# extglob
	# If set, the extended pattern matching features described above (see Pattern Matching)
	# are enabled.
	shopt -q -s extglob
	...
	# failglob
	# If set, patterns which fail to match filenames during filename expansion result in an
	# expansion error.
	shopt -q -u failglob
	...
	# (see Pattern Matching) behave as if in the traditional C locale when performing
	...
	# immediately passed to the shell parser. Instead, the resulting line is loaded into
	# the Readline editing buffer, allowing further modification.
	shopt -q -u histverify

	# hostcomplete
	# If set, and Readline is being used, Bash will attempt to perform hostname completion
	# when a word containing a ‘@’ is being completed (see Commands For Completion). This
	# option is enabled by default.
	...
	# Allow a word beginning with ‘#’ to cause that word and all remaining characters on
	# that line to be ignored in an interactive shell. This option is enabled by default.
	...
	# lithist
	# If enabled, and the cmdhist option is enabled, multi-line commands are saved to the
	...
	# login_shell
	# The shell sets this option if it is started as a login shell (see Invoking Bash). The
	...
	# nocaseglob
	# If set, Bash matches filenames in a case-insensitive fashion when performing filename
	# expansion.
	...
	# word expansions, or when filtering possible completions as part of programmable
	# completion.
	...
	# progcomp
	# If set, the programmable completion facilities (see Programmable Completion) are
	...
	if [ "${BASH_VERSINFO:-0}" -ge 5 ]; then
		...
		#it has an alias, Bash attempts programmable completion using the command word
	...
	#expansion, and quote removal after being expanded as described below
	#(see Controlling the Prompt). This option is enabled by default.
	shopt -q -s promptvars

	#restricted_shell
	#The shell sets this option if it is started in restricted mode (see The Restricted
	#Shell). The value may not be changed. This is not reset when the startup files are
