...
if [[ ${INST_HOSTNAME:-EMPTY} == "EMPTY" ]]; then
...
if [[ ${INST_ENABLE_ENCRYPTION^^} == "TRUE" ]]; then
...
if [[ ${INST_ENABLE_IMPERMANENCE^^} == "TRUE" ]]; then
...
nixos-install \
	...
	--show-trace \
	--no-root-passwd \
	--flake "${FLAKE_LOCATION:-.}#${INST_HOSTNAME}" \
	--impure \
	--root /mnt || {
