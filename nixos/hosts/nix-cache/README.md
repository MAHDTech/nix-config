...
host information, cache-info links and a link to `https://hub.slopageddon.app`.
The page is generated by Nix and uses local CSS, without JavaScript, external
assets, a timer or a metrics backend. It is not a live health indicator.

The exact root and stylesheet locations are separate from the existing
