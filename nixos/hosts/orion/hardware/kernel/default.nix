...
let
  ...
  kernelBuild = pkgs.stdenv.mkDerivation {
    ...
    # Apply consolidated Sky1-Linux rebased patch for 7.2-rc5
    # This is a single patch containing all 42 Sky1-Linux patches-next
    # (0001-0042) rebased onto Linux 7.2-rc5. Generated by:
    #   1. Downloading 7.2-rc5 source
    #   2. Applying patches-next sequentially (19 clean, 16 fuzz, 8 manual rebase)
    #   3. Generating consolidated diff
    # Patch 0043 (EMU/FPGA dead code cleanup) is deferred — non-critical cosmetic.
    prePatch = ''
      echo "Applying consolidated Sky1-Linux rebased patch for 7.2-rc5..."
      patch -p1 --fuzz=2 < ${./patches/sky1-7.2-rc5-all.patch}
    '';
