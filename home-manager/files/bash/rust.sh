...
function updateRust() {
	...
	if checkBin rustup >/dev/null 2>&1;
	then
		...
		then
			...
			if ! grep -q "Update available" <(RUSTUP_USE_CURL=1 rustup check) ;
			then
			...
			rm -f "${LOCK}" \
				|| { writeLog "ERROR" "Failed to remove rustup update lock" ; exit 1 ; }
...
function configureRustup() {
	...
	do
		...
		if grep -q "${TARGET} (installed)" <(rustup target list);
		then
