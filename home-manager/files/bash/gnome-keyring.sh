...
function set_vars_gnome_keyring() {
	...
	if [[ "${GNOME_KEYRING_CONTROL:EMPTY}" == "EMPTY" ]];
	then
		writeLog "ERROR" "The variable GNOME_KEYRING_CONTROL is empty!"
		return 1
	fi

	if [[ "${GNOME_KEYRING_LOCAL:EMPTY}" == "EMPTY" ]];
	then
	...
	if [[ -S "${XDG_RUNTIME_DIR}/keyring/ssh" ]];
	then
	...
	if [[ -S "${XDG_RUNTIME_DIR}/keyring/gpg" ]];
	then
...
function start_gnome_keyring() {
	...
	if type gnome-keyring 1>/dev/null 2>&1; then
		...
		if ! pgrep --full gnome-keyring-daemon >/dev/null;
		then
