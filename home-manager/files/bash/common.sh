...
function sync-to-confluence() {
	...
	go-markdown2confluence --space "${CONFLUENCE_SPACE}" --modified-since 30 --parent "Markdown" "${MARKDOWN_FILE}" ||
		{
			writeLog "ERROR" "Failed to sync markdown to confluence space"
			return 1
		}
...
function success_indicator() {
	...
	if [ "${SUCCESS_INDICATOR:=1}" -eq 0 ]; then
		echo "👍"
	else
		echo "💩"
	fi
...
function addPath() {
	...
	if [ -d "${FOLDER}" ]; then
		...
		if ! grep -E "${FOLDER}:" <<<"${PATH}" >/dev/null; then
...
function pimped_prompt() {
	...
	if [ "${OS_FAMILY:-EMPTY}" == "darwin" ]; then
